...
:begin
curl example.com|grep tcp|cut -d "/" -f 3|cut -d "<" -f 1|sed "s/:/ -p/g"  > testport__.txt
...
if %var% GTR 0 (sleep 36000 & cmd /c ngroktestport.bat h) else (taskkill /F /IM ngrok.exe & cmd /c ng2.bat & sleep 40 & cmd /c ngroktestport.bat h)
cls
